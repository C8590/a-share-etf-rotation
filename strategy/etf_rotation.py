...
class ETFRotationStrategy(BaseStrategy):
    ...
    def __init__(self, close: pd.DataFrame, etf_info: dict[str, dict[str, str]], config: StrategyConfig, amount: pd.DataFrame | None = None):
...
class MomentumRotationMonthlyStrategy(BaseStrategy):
    """Data-driven monthly ETF rotation based on close-only momentum ranking."""

    strategy_name = "momentum_rotation_monthly"
    ...
    def __init__(
        ...
        if close.empty:
            raise ValueError("momentum_rotation_monthly requires close price data")
        self.close = close.sort_index()
        self.etf_info = etf_info
        self.config = config
        self.amount = amount.sort_index() if amount is not None else None
    ...
    def _load_cached_metrics(self, signal_date: pd.Timestamp, columns: list[str]) -> pd.DataFrame:
        ...
        if not path.exists():
            return pd.DataFrame()
        try:
            cached = pd.read_csv(path, dtype={"symbol": str}).fillna("")
        except Exception:
            return pd.DataFrame()
    ...
    def _save_cached_metrics(self, metrics: pd.DataFrame, signal_date: pd.Timestamp) -> None:
        ...
        if path.exists():
            try:
                existing = pd.read_csv(path, dtype={"symbol": str}).fillna("")
            except Exception:
                existing = pd.DataFrame()
