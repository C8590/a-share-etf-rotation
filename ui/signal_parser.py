...
MAIN_STRATEGY = "日频右侧确认型 ETF 动量轮动策略"
STRATEGY_ORDER = [MAIN_STRATEGY]
...
DISPLAY_COLUMNS_SELL_DECISION = {
    "symbol": "ETF代码",
    "name": "ETF名称",
    "current_shares": "当前持有份额",
    "target_shares": "目标份额",
    "suggested_sell_shares": "建议卖出份额",
    "current_price": "当前参考价",
    "sell_type": "卖出类型",
    "sell_ratio": "建议卖出比例",
    "first_sell_price": "第一卖出价",
    "second_sell_price": "第二卖出价",
    "third_sell_price": "第三卖出价",
    "first_take_profit_price": "第一止盈价",
    "second_take_profit_price": "第二止盈价",
    "third_take_profit_price": "第三止盈价",
    "risk_trigger_price": "风控触发价",
    "risk_trigger_source": "触发价来源",
    "risk_trigger_ratio_to_current": "触发价/当前价",
    "risk_trigger_warning": "触发价说明",
    "risk_limit_price": "风控挂单价",
    "execution_note": "执行说明",
    "risk_note": "风险提示",
}

SELL_EXECUTION_COLUMN_ORDER = [
    "symbol",
    "name",
    "current_shares",
    "sell_type",
    ...
    "third_take_profit_price",
    "risk_trigger_price",
    "risk_trigger_source",
    "risk_trigger_ratio_to_current",
    "risk_trigger_warning",
    "execution_note",
    "risk_note",
]

SELL_EXECUTION_ALIASES = {
    "symbol": ["symbol", "ETF代码", "ETF 代码"],
    "name": ["name", "ETF名称", "ETF 名称"],
    "current_shares": ["current_shares", "当前持有份额", "持有份额"],
    "target_shares": ["target_shares", "目标份额"],
    "suggested_sell_shares": ["suggested_sell_shares", "建议卖出份额"],
    "current_price": ["current_price", "当前参考价", "当前价格"],
    "sell_type": ["sell_type", "卖出类型"],
    "sell_ratio": ["sell_ratio", "建议卖出比例", "卖出比例"],
    "first_sell_price": ["first_sell_price", "第一卖出价"],
    "second_sell_price": ["second_sell_price", "第二卖出价"],
    "third_sell_price": ["third_sell_price", "第三卖出价"],
    "first_take_profit_price": ["first_take_profit_price", "第一止盈价"],
    "second_take_profit_price": ["second_take_profit_price", "第二止盈价"],
    "third_take_profit_price": ["third_take_profit_price", "第三止盈价"],
    "risk_trigger_price": ["risk_trigger_display", "risk_trigger_price", "风控触发价", "趋势破坏线"],
    "risk_trigger_source": ["risk_trigger_source", "触发价来源", "风控触发来源"],
    "risk_trigger_ratio_to_current": ["risk_trigger_ratio_to_current", "触发价/当前价"],
    "risk_trigger_warning": ["risk_trigger_warning", "触发价说明"],
    "risk_limit_price": ["risk_limit_price", "风控挂单价"],
    "execution_note": ["execution_note", "执行说明", "卖出说明", "触发说明"],
    "risk_note": ["risk_note", "风险提示", "warning", "validation_warning"],
}
...
def source_label(source: str) -> str:
    if source == "manual":
        return "手动选择"
    if source == "latest_after_refresh":
        return "刷新后最新信号"
    return "自动使用最新可用数据"
...
def load_etf_names(project_root: Path) -> dict[str, str]:
    sector_map = load_etf_sector_map(project_root / "config" / "etf_sector_map.yaml")
    names = {
        str(code).zfill(6): str(record.get("name", "")).strip()
        for code, record in sector_map.items()
        if str(record.get("name", "")).strip() and str(record.get("name", "")).strip() != "名称未录入"
    }
    for candidate in [
        ...
        if candidate.exists():
            ...
            if {"symbol", "name"}.issubset(frame.columns):
                for _, row in frame.iterrows():
                    ...
                    if symbol not in names and str(row["name"]).strip():
    ...
    for item in items:
        if item.get("symbol"):
            ...
            if symbol not in names and str(item.get("name", "")).strip():
                names[symbol] = str(item.get("name", ""))
    return names
...
def _rankings_from_signal_rows(signals: pd.DataFrame) -> pd.DataFrame:
    if signals.empty or "rank_table" not in signals.columns:
        return pd.DataFrame()
    text = _clean_text(signals.iloc[0].get("rank_table"))
    if not text:
        return pd.DataFrame()
    try:
...
def build_overview(
    ...
    data_cutoff_date = _clean_text(main_row.get("data_cutoff_date", main_row.get("latest_data_date", "")))
    ...
    market_phase = _clean_text(main_row.get("market_phase", ""))
    ...
    return {
        ...
        "actual_signal_date": effective_signal_date or "N/A",
        "data_cutoff_date": data_cutoff_date or latest_data_date or "N/A",
        "execute_date": execute_date or "N/A",
        "execution_status": execution_status or "N/A",
        "signal_date_source": signal_date_source,
...
def load_dashboard_data(project_root: Path) -> DashboardData:
    ...
    rankings = _rankings_from_signal_rows(signals)
...
def _display_plan_frame(records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    money_cols = ["目标金额", "今日建议买入金额", "预计买入金额", "预计卖出金额", "当前估算金额", "一手所需资金", "当前可用现金", "建议买入金额", "持仓成本", "当前市值", "浮动盈亏"]
    share_cols = ["建议买入份额", "当前持有份额", "建议卖出份额", "当前份额", "持有份额", "第一档卖出份额", "第二档卖出份额", "第三档卖出份额"]
    price_cols = ["参考价格", "第一买入价", "第二买入价", "第三买入价", "买入价", "平均买入价", "当前价格", "第一止盈价", "第二止盈价", "第三止盈价", "风控触发价", "风控挂单价"]
    ...
    for col in money_cols:
        if col in df.columns:
            df[col] = df[col].apply(lambda value: _format_number(value, " 元", 2))
    for col in share_cols:
        if col in df.columns:
            df[col] = df[col].apply(lambda value: _format_number(value, " 份", 0))
    for col in price_cols:
        if col in df.columns:
            df[col] = df[col].apply(lambda value: _format_number(value, "", 3))
    if "浮动盈亏率" in df.columns:
        df["浮动盈亏率"] = df["浮动盈亏率"].apply(_format_pct_value)
    preferred = [
        "ETF 代码",
        "ETF 名称",
        "ETF代码",
...
def _format_display_number(value: Any, digits: int, suffix: str = "") -> str:
    if value in ("", None):
        return "-"
    try:
        if pd.isna(value):
...
def _format_sell_ratio(value: Any) -> str:
    if value in ("", None):
        return "0%"
    try:
        if pd.isna(value):
...
def _format_ratio_times(value: Any) -> str:
    if value in ("", None):
        return "-"
    try:
        if pd.isna(value):
...
def _sanitize_sell_display_text(value: Any) -> str:
    text = _clean_text(value, "-") or "-"
    return text.replace("风控卖出价", "风控触发价")


def _sell_execution_display_frame(records: list[dict[str, Any]]) -> pd.DataFrame:
    if not records:
        return pd.DataFrame()
    rows: list[dict[str, Any]] = []
    for record in records:
        record = validate_sell_prices(record)
        out: dict[str, Any] = {}
        for key in SELL_EXECUTION_COLUMN_ORDER:
            raw = _first_present(record, SELL_EXECUTION_ALIASES.get(key, [key]))
            title = DISPLAY_COLUMNS_SELL_DECISION[key]
            if key == "sell_type":
                out[title] = SELL_TYPE_LABELS.get(str(raw or ""), str(raw or "暂不卖出"))
            elif key == "sell_ratio":
                out[title] = _format_sell_ratio(raw)
            elif key == "risk_trigger_ratio_to_current":
                out[title] = _format_ratio_times(raw)
            elif key in {"current_shares", "target_shares", "suggested_sell_shares"}:
                out[title] = _format_display_number(raw, 0, " 份")
            elif key in {
                "current_price",
                "first_sell_price",
                "second_sell_price",
                ...
                out[title] = _format_display_number(raw, 3)
            else:
                out[title] = _sanitize_sell_display_text(raw)
        if out["卖出类型"] == SELL_TYPE_LABELS["hold"]:
            out["建议卖出比例"] = "0%"
            out["建议卖出份额"] = "0 份"
            out["第一卖出价"] = "-"
            out["第二卖出价"] = "-"
            out["第三卖出价"] = "-"
        rows.append(out)
    return pd.DataFrame(rows, columns=[DISPLAY_COLUMNS_SELL_DECISION[key] for key in SELL_EXECUTION_COLUMN_ORDER])


def parse_sell_execution_table(row: pd.Series) -> pd.DataFrame:
    return _sell_execution_display_frame(_json_records(row.get("sell_execution_plan")))
...
def parse_rank_table(row: pd.Series) -> pd.DataFrame:
    ...
    if "最终信号" in df.columns:
        df["最终信号"] = df["最终信号"].replace(
            {
                "selected": "进入候选池",
    ...
    for col in ["是否高于均线", "是否进入候选池"]:
        if col in df.columns:
            df[col] = df[col].apply(_bool_text)
    columns = [
        "ETF 代码",
        "ETF 名称",
        "交易所",
